...
std::shared_ptr<ContactSchedule> QuadrupedalGaitGenerator::walk(StdVec_Map_string_SE3 contacts, int N_ds, int N_ss,
                                                                int N_uds = 0, int N_uss = 0, double stepHeight = 0.15,
                                                                bool startPhase = true, bool endPhase = true) {
  if (contacts.size() != 2) {
    throw std::runtime_error("The size of the contact should be 2 (starting and ending position0");
  }
  try {
  ...
  std::shared_ptr<FootTrajectoryWrapper> lhSwingTraj =
      std::make_shared<FootTrajectoryWrapper>(dt_, N_ss, stepHeight, contacts[0][lh_], contacts[1][lh_]);
  std::shared_ptr<FootTrajectoryWrapper> rfSwingTraj =
      std::make_shared<FootTrajectoryWrapper>(dt_, N_ss, stepHeight, contacts[0][rf_], contacts[1][rf_]);
  ...
  std::vector<std::shared_ptr<ContactPhase>> rh_schedule = {
      ...
      std::make_shared<ContactPhase>(N - (N_0 + N_ds + 3 * N_ss - N_uss - N_uds))};
...
std::shared_ptr<ContactSchedule> QuadrupedalGaitGenerator::trot(StdVec_Map_string_SE3 contacts, int N_ds, int N_ss,
                                                                int N_uds = 0, int N_uss = 0, double stepHeight = 0.15,
                                                                bool startPhase = true, bool endPhase = true) {
  if (contacts.size() != 2) {
    throw std::runtime_error("The size of the contact should be 2 (starting and ending position0");
  ...
  std::vector<std::shared_ptr<ContactPhase>> lh_schedule = {std::make_shared<ContactPhase>(N_0),
                                                            std::make_shared<ContactPhase>(N_ss, lhSwingTraj),
                                                            std::make_shared<ContactPhase>(N - (N_0 + N_ss))};
  std::vector<std::shared_ptr<ContactPhase>> rf_schedule = {std::make_shared<ContactPhase>(N_0),
                                                            std::make_shared<ContactPhase>(N_ss, rfSwingTraj),
                                                            std::make_shared<ContactPhase>(N - (N_0 + N_ss))};
  std::vector<std::shared_ptr<ContactPhase>> lf_schedule = {
      std::make_shared<ContactPhase>(N_0 + N_ss - N_uss), std::make_shared<ContactPhase>(N_ss, lfSwingTraj),
      std::make_shared<ContactPhase>(N - (N_0 + 2 * N_ss - N_uss))};
  std::vector<std::shared_ptr<ContactPhase>> rh_schedule = {
      std::make_shared<ContactPhase>(N_0 + N_ss - N_uss), std::make_shared<ContactPhase>(N_ss, rhSwingTraj),
      std::make_shared<ContactPhase>(N - (N_0 + 2 * N_ss - N_uss))};
